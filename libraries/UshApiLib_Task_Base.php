...
class UshApiLib_Task_Base extends UshApiLib_Ushahidi_API_Library_Base
 {
    ...
    protected function parse_json()
    {
    	...
    	if(!isset($data_array[UshApiLib_Task_Base::ERROR_INDEX][UshApiLib_Task_Base::ERROR_CODE_INDEX]))
    	{
    		return new UshApiLib_Task_Response_Base("U1", "Unable to parse JSON string. use getJson() on the task object to see what was returned");
    	}
    	if(!isset($data_array[UshApiLib_Task_Base::ERROR_INDEX][UshApiLib_Task_Base::ERROR_MESSAGE_INDEX]))
    	{
    		return new UshApiLib_Task_Response_Base("U1", "Unable to parse JSON string. use getJson() on the task object to see what was returned");
    	}
